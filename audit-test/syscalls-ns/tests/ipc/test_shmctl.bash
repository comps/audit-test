...
# IPC-1A
eval_syscall pass 0 do_shmctl "$shmid_initns" stat
# IPC-1B, errno 22 = EINVAL, errno 43 = EIDRM
eval_syscall fail 22,43 exec_ns do_shmctl "$shmid_initns" stat

# IPC-2A, errno 22 = EINVAL, errno 43 = EIDRM
eval_syscall fail 22,43 do_shmctl "$shmid_newns" stat
# IPC-2B
eval_syscall pass 0 exec_ns do_shmctl "$shmid_newns" stat

# due to the nature of sysv ipc ids (msqid,semid,shmid), IPC-3 is not tested
# here - see README of this bucket for more info

# IPC-3A
#eval_syscall pass 0 dropcap cap_ipc_owner do_shmctl "" stat
# IPC-3B, errno 1 = EPERM, errno 13 = EACCES
#eval_syscall fail 1,13 exec_ns dropcap cap_ipc_owner do_shmctl "" stat
