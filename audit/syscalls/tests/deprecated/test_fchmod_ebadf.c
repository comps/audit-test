 ...
 **   2) Clear the audit trail
 **   3) Execute the "fchmod" system call on a file that does not exist
 **   4) Tests the results of the system call against the
 **      expected successful return
 **      
 **  The erroneous case attempts to change the mode on a file that does
...
int test_fchmod_ebadf(laus_data* dataPtr) {
