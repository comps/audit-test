/**********************************************************************
  ...
  **
  **
  **  FILE       : test_getxattr.c
  **
  **  PURPOSE    : To test the getxattr library call auditing.
  **
  **  DESCRIPTION: The test_getxattr() function builds into the
  ...
  **   3) Calls getxattr on the temporary file
  **   4) Verifies the success result.
  **
  **  The successful case passes a valid path, name, and size to the
  **  getxattr call, thus satisfying the conditions as given in the man
  **  page for getxattr for a success result.
  **  
  **  In the erroneous case, this function:
  **   1) Calls getxattr with path=NULL
  **   2) Verifies the error result.
  **      
  **  The erroneous case causes an EFAULT as detailed in the man page in
  **  stat(2) by passing an invalid address (NULL) to getxattr.
  **
  **  HISTORY    :
 ...
 int test_getxattr(laus_data* dataPtr) {
   ...
   dataPtr->laus_var_data.syscallData.code = __NR_getxattr;
   ...
   // Execute system call
   dataPtr->laus_var_data.syscallData.result = syscall( __NR_getxattr, path, name, value, size );
