/**********************************************************************
   ...
   **
   **
   **  FILE       : test_lchown16_efault.c
   **
   **  PURPOSE    : To test the lchown16 library call auditing.
   **
   **  DESCRIPTION: The test_lchown16() function builds into the
   **  laus_test framework to verify that the Linux Audit System
   **  accurately logs both successful and erroneous execution of the
   **  "lchown16" system call.
   **
   **  In the successful case, this function:
   **   1) Generate a file name and create a 777 test file owned by root
   **   2) Clear the audit trail
   **   3) Execute the "lchown16" system call to change ownership to test user
   ...
   **  As the file does exist, and the mode is 777, the lchown16 syscall
   **  should succeed.
   **  
   **  In the erroneous case, this function:
   **   1) Generate a file name and do not create the test file
   **   2) Clear the audit trail
   **   3) Execute the "lchown16" system call on a file that does not exist
   **   4) Tests the results of the system call against the
   **      expected successful return
   **      
   ...
   **********************************************************************/
   ...
   int test_lchown16_efault(laus_data* dataPtr) {
     ...
     // Execute system call
     dataPtr->laus_var_data.syscallData.result = syscall( __NR_lchown16, linkName, owner, group );
