/**********************************************************************
   ...
   **             1) Generates a unique file name
   **             2) Creates the temporary filename
   **             3) Clears the audit trail
   **             4) Uses the "mknod" system call on the filename that exists
   ...
   **            described by the "EFAULT" error under the "mknod" system
   **            system call manpage.  That is, the mknod() function is
   **            called using a directory that already exists, and thus the
   **            operation fails.
   **
   **           
   **
   ...
   **********************************************************************/
   ...
   int test_mknod_efault(laus_data* dataPtr) {
     ...
     //hard cod the size of dev because dev_t in the kernel is different
