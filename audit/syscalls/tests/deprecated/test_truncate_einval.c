/**********************************************************************
   ...
   **             1) Creates the temporary file
   **             2) Executes the "truncate" system call with an invalid
   **                length parameter
   **
   **            The erroneous case executes the faulty conditions
   **            described by the "EINVAL" error under the "truncate" system
   ...
   */
   int test_truncate_einval(laus_data* dataPtr) {
