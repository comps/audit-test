 /**********************************************************************
    ...
    **  FILE       : test_setuid.c
    **
    **  PURPOSE    : To test the setuid library call auditing.
    **
...
int test_setuid(laus_data *dataPtr)
{
    ...
    if ((rc = auditArg1(dataPtr, AUDIT_ARG_IMMEDIATE, sizeof(int), &uid)) != 0) {
    ...
    dataPtr->laus_var_data.syscallData.result = syscall(__NR_setuid, uid);
