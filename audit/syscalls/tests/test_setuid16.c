 /**********************************************************************
    ...
    **
    **  PURPOSE    : To test the setuid16 library call auditing.
    **
...
int test_setuid16(struct audit_data *context)
{
    int rc = 0;
    int exp_errno = EPERM;
    int uid;
    ...
    context->u.syscall.sysnum = AUDIT_setuid;
	...
	context->fsuid = 0;
	context->fsgid = 0;
	printf5("Target uid=%d in success case\n", uid);
    } else {
    ...
    if ((rc = auditArg1(context, AUDIT_ARG_IMMEDIATE, sizeof(uid), &uid)) != 0) {
    ...
    context->u.syscall.exit = syscall(__NR_setuid16, uid);
