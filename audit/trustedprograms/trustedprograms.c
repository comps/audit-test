...
**
**
**  FILE   : trustedprograms.c
**
**  PURPOSE: This file contains the main body of the Linux Auditing 
**           System test suite for trusted programs.  This file, as well as 
...
**           test functions for separate trusted programs (located in the
**           ./tests/ directory) and these functions simply plug into
**           this framework.  
**           
**           As the tests become available, pointers to these
**           functions are added to the trustedprogramTests[] array.  Each
...
Arguments:\n\
  ...
  -t $TRUSTEDPROGRAM:   Optionally specify a single test case to run.\n\
                 $TRUSTEDPROGRAM is the name of the trusted program to test.\n\
...
/**
 * MH: These tests are failing randomly, so we are taking them out and
 * having them done manually.
 */
//    {&test_at, "at", NULL},
//    {&test_atd, "atd", NULL},
//    {&test_cron, "cron", NULL},
//    {&test_crontab, "crontab", NULL},
    {&test_gpasswd, "gpasswd", NULL},
    {&test_groupadd, "groupadd", NULL},
    {&test_groupdel, "groupdel", NULL},
    {&test_groupmod, "groupmod", NULL},
    {&test_passwd, "passwd", NULL},
    {&test_useradd, "useradd", NULL},
    {&test_userdel, "userdel", NULL},
	...
	login_uid = login_uid_data->pw_uid;
      ...
      case 't':
        ...
        for (k = 0; k < sizeof(trustedprogramTests)/sizeof(trustedprogram_data); k++) {
          if ((rc = strcmp(testcase, trustedprogramTests[k].testName)) == 0 ) {
            break;
          }
        }
        if ( rc != 0 ) {
  ...
  */
  ...
  for (k = 0; k < sizeof(trustedprogramTests)/sizeof(trustedprogram_data); k++) {
    ...
    if ((testcase != NULL) && 
	(strcmp(testcase, trustedprogramTests[k].testName) != 0 )) {
      continue;
    }
