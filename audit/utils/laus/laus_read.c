...
int close_audit_log(struct aud_log *audit_log)
{
	...
	if (audit_log->start != NULL) {
	    munmap(audit_log->start - sizeof(struct laus_file_header),
		   audit_log->st_size);
	}
...
struct aud_message_on_disk *get_next_audit_record(struct aud_log *audit_log,
						  struct
						  laus_record_header_on_disk
						  *hdr)
{
    //int i;
	...
	printf
	    (" >>> log pointer (%p) plus header rsize (%zx) greater than end of log (%p) <<<\n",
