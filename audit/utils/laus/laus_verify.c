...
//generic
static void msg_info(int, const struct aud_message_on_disk *,
		     const laus_data *);
...
#define aud_msg_exit_on_disk aud_msg_exit
  ...
  AUDIT_BEGIN:
	...
	switch (dataPtr->msg_type) {
		...
		if (dataPtr->laus_var_data.syscallData.personality ==
		    NO_CHECK_SYSCALL_DATA) {
		    printf4
			("Flag set indicating that no log verify be performed for this syscall.");
		    continue;
		}
		...
		if ((sycl->personality !=
		    ...
		    printf9("\tBegin Range: %s",
			    ctime(&(dataPtr->begin_r_time)));
		...
		if ((dataPtr->laus_var_data.syscallData.result !=
		     NO_RETURN_CODE)
		    &&
		    ...
		    printf9("Message:    %s", ctime(&(r_time)));
		    printf9
			("\tactual result:expected result:expected errno\t%i:%i:%i\n",
			 sycl->result,
			 dataPtr->laus_var_data.syscallData.result,
			 dataPtr->laus_var_data.syscallData.resultErrno);
		    continue;
		...
		if (compare_var_parms(sycl->data,
				      dataPtr->laus_var_data.syscallData.
				      data) != 0) {
		...
		if (dataPtr->msg_evname[0] != '\0'
		    && (rc =
			strcmp(msg->msg_evname, dataPtr->msg_evname)) != 0) {
		...
		if ((rc = strncmp(usrdata,
				  dataPtr->laus_var_data.textData.data, len))
		    != 0) {
		    printf9("audit text didn't match\n");
		    printf9("\tActual  : %.*s\n\t\tExpected: %s\n", len,
			    usrdata, dataPtr->laus_var_data.textData.data);
		    continue;
		}
		...
		if (login->uid != dataPtr->laus_var_data.loginData.uid ||
		    strcmp(login->hostname,
			   dataPtr->laus_var_data.loginData.hostname) != 0 ||
		    strcmp(login->address,
			   dataPtr->laus_var_data.loginData.address) != 0 ||
		    strcmp(login->terminal,
			   dataPtr->laus_var_data.loginData.terminal) != 0 ||
		    strcmp(login->executable,
			   dataPtr->laus_var_data.loginData.executable) != 0) {
		    printf9("login data didn't match\n");
		    printf9("Field:      Actual\tExpected:\n");
		    printf9("uid:        %d\t%d\n", login->uid,
			    dataPtr->laus_var_data.loginData.uid);
		...
		if (netlink->result !=
		    dataPtr->laus_var_data.netlinkData.result) {
	    ...
	    default:
		printf1
		    ("ERROR: potentially matching message found but this function can currently only handle system call, text, login, exit and netlink message types\n");
		goto OUT;
		break;
	}
...
static
void msg_info(int level, const struct aud_message_on_disk *msg,
	      const laus_data *dataPtr)
{
...
static
void sycl_info(int level, const struct aud_msg_syscall *sycl,
	       const laus_data *dataPtr)
{
